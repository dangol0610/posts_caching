...
| PostgreSQL | latest | 5433 | Основное хранилище постов |
| Redis | latest | 6377 | Кэш постов |
...
2. При hit — возврат из кэша
3. При miss — запрос в БД + запись в кэш (TTL: 1 час)
...
Docker контейнеры должны быть запущены для интеграционных тестов.
...
3. **Актуальность данных** — инвалидация при удалении, обновление кэша при обновлении
